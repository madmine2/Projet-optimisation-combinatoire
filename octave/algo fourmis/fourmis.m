...
##     endif
 ...
 # parametre de l'algo ------------------------------------------
    ...
    alpha = 5;                                            # Importance du pheromone (plus grand que 1) (grand alpha = plus de pheromone)
    beta = 0.9 ;                                          # importance de la visibilite (plus petit que 1) (grand beta = plus de pheromone)
    Q = 0.9;                                              # poids du pheromone depose (grand Q = plus de pheromone)
    omega = 0.2;                                          # intensite de l'evaporation du pheromone (entre 0 et 1) (grand omega = moins de pheromone)
 ...
 for epoque = 1 : tMax
    ...
    for fourmi = 1 : nombreFourmis  
       ...
       coutTrajetFourmi = 0;                              # cout du trajet de cette fourmi
        listeVilleEncoreDisponibles = ring;               # toutes les villes vers lequelles la fourmis peut aller
         # Pour chaque ville (on les prends dans un ordre aléatoire) on va choisir une autre ville pour faire le trajet en fonction des pheromone
          ...
          for i = 1 :(nombreLieu )
            listeTrajetFourmi = [listeTrajetFourmi villeActuelle];
            listeVilleEncoreDisponibles = listeVilleEncoreDisponibles(listeVilleEncoreDisponibles ~= ring(villeActuelle)); # permet de retirer la villeActuelle de la liste (me demande pas comment ça marche)
            [villeActuelle, listeVilleEncoreDisponibles,coutTrajetFourmi] = choixDuTrajetIVersJ(coutTrajetFourmi,ring,villeActuelle,nombreLieu, matricePheromone,matriceDeltaPheromone, cost_ring, alpha, beta,listeVilleEncoreDisponibles); #calcul la probabilité de choisir chaque chemin et ensuite utilise random
          endfor  
       ...
       for m = 1 : nombreLieu - 1
        ...
        matriceDeltaPheromone(listeTrajetFourmi(1),listeTrajetFourmi(end)) += deltaTau;     # Ajout pour le trajet du dernier au premier
        matriceDeltaPheromone(listeTrajetFourmi(end),listeTrajetFourmi(1)) += deltaTau;
    endfor
  ...
  for v = 1 : nombreLieu
   new_ring(v) = ring(listeTrajetFourmi(v));
  endfor
